...
public class ArcadeController implements Initializable {
   ...
   public void initialize(URL url, ResourceBundle rb) { 
       list=(ArrayList<IGameObject>) LevelGenerator.makeObjects(new  ArcadeMode(),12,Math.round((float) canv.getWidth()), (int) Math.round(canv.getHeight()),250,0.07);
            x=list.get(0).getPositionX();
            y=list.get(0).getPositionY();
             gc=canv.getGraphicsContext2D();
            slist=new ArrayList<>();
            imag=new ArrayList<>();
            highscore.setText(" : "+user.getScoreArcade());
timer = new AnimationTimer() {
    ...
    public void handle(long now) {
       ...
       if (slist.get(0).isfreeze())
        ...
        if (Timer==100)
        {
                GameObject.updatetime(1);
                if (GameObject.getTime()<10)
                    time.setText(" :0:0" +GameObject.getTime());
                else
              time.setText(" :0:" +GameObject.getTime());
              Timer=0;
        }
        if (slist.size()==0)
        {
            for (int i=0;i<random.nextInt(6)+1&&i<list.size();i++)
            {
        ...
        for (int i=0;i<slist.size();i++)
        {
            ...
            {
                timer.stop();
                user.setMaxScore(4, list.get(0).getScore());
                gc.clearRect(0, 0, canv.getWidth(), canv.getHeight());
     ...
     for (int i=0;i<slist.size();i++)
        {
         ...
         score.setText(": "+list.get(0).getScore());
            if (list.get(0).getScore()>user.getScoreArcade())
