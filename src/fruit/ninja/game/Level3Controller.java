...
public class Level3Controller implements Initializable {
   ...
   public void initialize(URL url, ResourceBundle rb) { 
      list=(ArrayList<IGameObject>) LevelGenerator.makeObjects(new  Classicmode(),1,Math.round((float) canv.getWidth()), (int) Math.round(canv.getHeight()),250,0.1);
            x=list.get(0).getPositionX();
            y=list.get(0).getPositionY();
            gc=canv.getGraphicsContext2D();
            slist=new ArrayList<>();
            imag=new ArrayList<>();
            highscore.setText( ": "+user.getScoreLevelThree() );
timer = new AnimationTimer() {
    ...
    public void handle(long now) {
       ...
       if (slist.get(0).isfreeze())
        ...
        if (slist.size()==0)
        {
            for (int i=0;i<random.nextInt(4)&&i<list.size();i++)
            {
        ...
        for (int i=0;i<slist.size();i++)
        {
             ...
             else if (list.get(0).getNumOfSlices()>=2)
         {
            ...
            {
                timer.stop();
                   user.setMaxScore(3, list.get(0).getScore());
                gc.clearRect(0, 0, canv.getWidth(), canv.getHeight());
    ...
    public void handle(MouseEvent event) {
        ...
        for (int i=0;i<slist.size();i++)
        {
         ...
         score.setText(" : "+list.get(0).getScore());
            if (list.get(0).getScore()>user.getScoreLevelThree())
