...
public class FXMLDocumentController implements Initializable {
 ...
 private audio audio = new audio();
   ...
   public void initialize(URL url, ResourceBundle rb) { 
        System.out.println(user.getScoreLevelOne());
       list=(ArrayList<IGameObject>) LevelGenerator.makeObjects(new  Classicmode(),1,Math.round((float) canv.getWidth()), (int) Math.round(canv.getHeight()),250,0.05);
            x=list.get(0).getPositionX();
            y=list.get(0).getPositionY();
         gc=canv.getGraphicsContext2D();
            slist=new ArrayList<>();
            imag=new ArrayList<>();
            highscore.setText(" : "+user.getScoreLevelOne());
       ...
       if (slist.get(0).isfreeze())
        ...
        for (int i=0;i<slist.size();i++)
        {
             ...
             else if (list.get(0).getNumOfSlices()>=2)
         {
            ...
            {
                timer.stop();
                user.setMaxScore(1, list.get(0).getScore());
                gc.clearRect(0, 0, canv.getWidth(), canv.getHeight());
       ...
       for (int i=0;i<slist.size();i++)
        {
            ...
            else if (slist.get(i)instanceof SpecialCantaloupe)
            {
                    ...
                    {
                        if (slist.get(i2)instanceof Apple || slist.get(i2)instanceof Banana || slist.get(i2)instanceof Orange || slist.get(i2)instanceof SpecialMango&&!slist.get(i2).isSliced())
                        {
                        slist.get(i2).slice();
         ...
         score.setText(": "+list.get(0).getScore());
            if (list.get(0).getScore()>user.getScoreLevelOne())
                    highscore.setText(" :" + list.get(0).getScore());
    }
};
canv.setOnMouseDragged(c);
